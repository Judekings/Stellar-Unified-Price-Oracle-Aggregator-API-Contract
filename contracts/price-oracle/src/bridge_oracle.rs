...
use crate::events::{
    emit_admin_action, BridgeOracleRegisteredEvent, BridgePriceSubmittedEvent,
};
use crate::storage::{get_admin, LEDGER_BUMP, LEDGER_THRESHOLD};
use crate::types::{DataKey, BridgedPrice, BridgeOracleConfig, ErrorCode};
...
pub fn get_bridge_oracle(env: &Env, source_asset: Address, target_asset: Address) -> Option<BridgeOracleConfig> {
...
pub fn submit_bridged_price(env: &Env, source_asset: Address, target_asset: Address, price: i128, timestamp: u64) {
    ...
    env.storage()
        .persistent()
        .set(&DataKey::BridgedPrice(source_asset.clone(), target_asset.clone()), &entry);
    env.storage()
        .persistent()
        .extend_ttl(&DataKey::BridgedPrice(source_asset.clone(), target_asset.clone()), LEDGER_THRESHOLD, LEDGER_BUMP);
...
pub fn normalize_bridged_price(env: &Env, raw_price: i128, target_decimals: u32, config: &BridgeOracleConfig) -> i128 {
...
pub fn get_bridged_price(env: &Env, source_asset: Address, target_asset: Address) -> Option<BridgedPrice> {
